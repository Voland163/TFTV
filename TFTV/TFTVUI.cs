        ...
        internal class MissionDeployment
        {
            ...
            internal class BaseDefense
            {
                ...
                private static void SetBaseDefenseSitrep(GeoMission mission)
                {
                    try
                    {
                        _timer = TFTVBaseDefenseGeoscape.CalculateBaseAttackProgress(mission);
                ...
                internal static void CreateCheckButton(GeoRosterDeploymentItem geoRosterDeploymentItem)
                {
                    try
                    {
                      ...
                      //  TFTVLogger.Always($"{geoRosterDeploymentItem.Character.DisplayName} vehicle? {isVehicle}");
                       ...
                       /* if (timer > 12)
                        {
                            uIButtonIconController.Icon.sprite = iconHangar;
                        }
                        else if (timer <= 12)
                        {
                            uIButtonIconController.Icon.sprite = iconEntrance;
                ...
                public static class TFTV_UIStateRosterDeployment_SetUpInitialDeployment0_patch
                {
                    public static void Postfix(UIStateRosterDeployment __instance, List<GeoRosterDeploymentItem> ____deploymentItems)
                    {
                        try
                        {
                            ...
                            SetBaseDefenseSitrep(__instance.Mission);

                            foreach (GeoRosterDeploymentItem geoRosterDeploymentItem in ____deploymentItems)
                            {
            ...
            internal class Behemoth
            {
                ...
                public static class TFTV_UIStateRosterDeployment_SetUpInitialDeployment_patch
                {
                    public static void Postfix(UIStateRosterDeployment __instance, List<GeoRosterDeploymentItem> ____deploymentItems)
                    {
                        try
                        {
                            if (!__instance.Mission.MissionDef.Tags.Contains(DefCache.GetDef<MissionTagDef>("Behemoth_MissionTagDef")))
                            {
                                return;
                            }
                            ...
                            Behemoth.listTeamA.Clear();
                            Behemoth.listTeamB.Clear();
