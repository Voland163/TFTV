...
{
    /// <summary>
    /// The one hover tooltip the personnel screen uses. It builds itself lazily on the panel's own
    /// canvas and places itself beside whatever element asked for it, so every icon on the screen -
    /// affinities, abilities, gear - explains itself the same way.
    /// </summary>
    internal static class PersonnelTooltip
    {
        private const string TooltipObjectName = "TFTV_PersonnelTooltip";
        ...
        internal static void Show(string content, Transform anchor)
        {
            try
            {
                if (string.IsNullOrEmpty(content) || anchor == null)
                {
                    return;
                }

                RectTransform tooltip = EnsureTooltip(anchor);
                if (tooltip == null)
                {
                    return;
                }
                ...
                PositionNextTo(tooltip, anchor as RectTransform);
        ...
        /// <summary>
        /// Anchors the tooltip to the left of the element and keeps it inside the canvas.
        /// </summary>
        private static void PositionNextTo(RectTransform tooltip, RectTransform anchorRect)
        {
            ...
            // corners[1] is the top-left corner of the anchor.
        ...
        /// <summary>
        /// The cached tooltip belongs to the panel's canvas, so it dies with the panel; Unity's null
        /// check catches that and the next hover builds a fresh one.
        /// </summary>
    ...
    /// <summary>
    /// Hover trigger for any element that carries a fixed piece of explanatory text.
    /// </summary>
    internal sealed class PersonnelTooltipTrigger : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
    {
        internal string Content;

        public void OnPointerEnter(PointerEventData eventData)
        {
            PersonnelTooltip.Show(Content, transform);
        }
