...
namespace TFTV
{
    internal class TFTVRevenant
    {
        ...
        public static PassiveModifierAbilityDef RevenantAbility; //= DefCache.GetDef<PassiveModifierAbilityDef>("Revenant_AbilityDef");
        public static DamageMultiplierStatusDef RevenantResistanceStatus; // = DefCache.GetDef<DamageMultiplierStatusDef>("RevenantResistance_StatusDef");
        ...
        // private static AddArmorStackEffectDef _addArmorStackEffectDef;
        private static ArmorStackStatusDef _armorStackStatusDef;
        ...
        internal class InternalData
        {
            public static void RevenantDataToClearOnStateChangeAndLoad()
            {
                try
                {
                    DeadSoldiersDelirium = new Dictionary<int, int>();
            ...
            public static void RevenantDataToClearOnLoadOnly()
            {
                try
                {
            ...
            public static void RevenantDataToClearOnMissionRestartOnly()
            {
                try
                {
                    revenantSpawned = false;
        ...
        internal class RevenantDefs
        {
            ...
            public static void CreateRevenantDefs()
            {
                try
                {
                    ...
                    CreateSpecialResistanceArmorStack();
            ...
            private static void CreateSpecialResistanceArmorStack()
            {
                try
                {
                    /*  string nameEffectDef = "RevenantAddArmorStack_EffectDef";
                      _addArmorStackEffectDef 
                          = Helper.CreateDefFromClone(
                              DefCache.GetDef<AddArmorStackEffectDef>("AddArmorStack_EffectDef"), 
                          "{07787E76-D564-4B82-A328-1297CFCC4C9E}", 
                          nameEffectDef);

                      _addArmorStackEffectDef.ApplicationConditions = new EffectConditionDef[] { };
                      _addArmorStackEffectDef.ArmorStackPointsToAdd = 1;*/

                    string nameStatusDef = "Revenant_ArmorStackStatusDef";
                    ArmorStackStatusDef source = DefCache.GetDef<ArmorStackStatusDef>("ArmorStack_StatusDef");

                    _armorStackStatusDef = Helper.CreateDefFromClone(source, "{37F2B658-CDAB-4C46-A454-6FC34D3E621A}", nameStatusDef);
                    _armorStackStatusDef.Visuals = Helper.CreateDefFromClone(source.Visuals, "{EE2703E5-50F3-4BB4-ABEA-698918746C95}", nameStatusDef);
                    _armorStackStatusDef.Visuals.DisplayName1.LocalizationKey = "KEY_TFTV_REVENANT_ARMOR_STACK_NAME";
                    _armorStackStatusDef.Visuals.Description.LocalizationKey = "KEY_TFTV_REVENANT_ARMOR_STACK_DESCRIPTION";
                    _armorStackStatusDef.EffectName = "RevenantSpecialResistanceArmorStack";
                    _armorStackStatusDef.SingleInstance = true;
                    _armorStackStatusDef.StackMultipleStatusesAsSingleIcon = true;

                    _armorStackStatusDef.ArmorStackDamageMultiplier = 0.5f;
                    _armorStackStatusDef.ArmorStackCap = 1;
                }
                catch (Exception e)
                {
                    TFTVLogger.Error(e);
            ...
            public static void CreateRevenantStatusEffect()
            {
                try
                {
                    ...
                    newAbilityStatusDef.AbilityDef = RevenantAbility;
            ...
            public static void CreateRevenantAbility()
            {
                try
                {
                    ...
                    RevenantAbility = revenantAbility;
            ...
            public static void CreateRevenantResistanceStatus()
            {
                try
                {
                    ...
                    RevenantResistanceStatus = revenantResistance;
        ...
        internal class StatsAndClasses
        {
            ...
            public static void IncreaseRevenantHP(TacticalActor tacticalActor)
            {
                try
                {
                    TFTVLogger.Always($"is actor Scylla with health below max? {tacticalActor.GameTags.Contains(queenTag)} {tacticalActor.Health.IntValue} {tacticalActor.Health.IntMax}");

                    if (tacticalActor.GameTags.Contains(queenTag) && tacticalActor.Health.IntValue < tacticalActor.Health.IntMax)
                    {
                        return;
                    }
        ...
        internal class Resistance
        {
            ...
            public static void ApplySpecialRevenantResistanceArmorStack(TacticalLevelController controller, TacticalFaction faction)
            {
                try
                {
                    if (controller.Factions.Any(f => f.Faction.FactionDef.MatchesShortName("aln")) &&
                        controller.GetFactionByCommandName("aln").TacticalActors.Any(ta => ta.IsAlive && ta.Status.HasStatus(RevenantResistanceStatus)) &&
                    ...
                    if (RevenantResistanceStatus != null && RevenantResistanceStatus.DamageTypeDefs[0] == null)
                    {
                        TFTVLogger.Always($"Apply special resistance armor stack, if applicable");
                        foreach (TacticalActor tacticalActor in controller.GetFactionByCommandName("aln").TacticalActors.Where(ta => ta.IsAlive && ta.Status.HasStatus(RevenantResistanceStatus)))
                        {
                            if (!tacticalActor.Status.HasStatus<ArmorStackStatus>())
                            {
                                tacticalActor.Status.ApplyStatus<ArmorStackStatus>(_armorStackStatusDef);
                            }

                            ArmorStackStatus armorStackStatus = tacticalActor.Status.GetStatus<ArmorStackStatus>();

                            if (armorStackStatus != null && armorStackStatus.BlockedAttacks != 1)
                            {
                                armorStackStatus.BlockedAttacks = 1;
                            }
                        }
                    }
                }
            ...
            public static void AddRevenantResistanceStatus(TacticalActorBase tacticalActorBase)
            {
                try
                {
                    ...
                    if (!tacticalActor.Status.HasStatus(RevenantResistanceStatus) && !tacticalActor.GameTags.Last().name.Contains("Mindfragged"))
                    {
                        ...
                        if (RevenantResistanceStatus.DamageTypeDefs[0] == null)
                        {
                            // TFTVLogger.Always($"Special resistance; applying armor stack to {tacticalActorBase.name}");
                            tacticalActor.Status.ApplyStatus<ArmorStackStatus>(_armorStackStatusDef);
                            ArmorStackStatus armorStackStatus = tacticalActor.Status.GetStatus<ArmorStackStatus>();
                            armorStackStatus.BlockedAttacks = 1;
                            // Effect.Apply(Repo, _addArmorStackEffectDef, TacUtil.GetActorEffectTarget(tacticalActor), null);            
                        }
            ...
            public static void TryToSpawnRevenant(TacticalLevelController controller)
            {
                try
                {
                    if (controller.GetFactionByCommandName("aln") != null)
                    {
                        ...
                        TFTVLogger.Always($"is actor Scylla with health below max? {actor.GameTags.Contains(queenTag)} {actor.Health.IntValue} {actor.Health.IntMax}");

                        if (actor.GameTags.Contains(queenTag) && actor.Health.IntValue < actor.Health.IntMax)
                        {
        ...
        internal class UIandFX
        {
            ...
            /*    [HarmonyPatch(typeof(TacticalActorBase), "UpdateClassViewElementDefs")]
                public static class UIModuleObjectives_UpdateClassViewElementDefs_patch
                {
                    public static void Postfix(TacticalActorBase __instance, ref List<ViewElementDef> ____classViewElementDefs)
                    {
                        try
                        {
                            ...
                            if (__instance.GetAbilityWithDef<PassiveModifierAbility>(RevenantAbility) != null)
                            {

                                if (!____classViewElementDefs.Contains(RevenantAbility.ViewElementDef)) 
                                {
                                    List<ViewElementDef> newList = new List<ViewElementDef>() { RevenantAbility.ViewElementDef, ____classViewElementDefs.ElementAtOrDefault(0)};

                                    ____classViewElementDefs = newList;
                                }

                                foreach (ViewElementDef viewElementDef in ____classViewElementDefs)
                                {
                                    TFTVLogger.Always($"{__instance?.name} {viewElementDef?.name}", false);
                ...
                public static void AddRevenantStatusEffect(TacticalActorBase actor)
                {
                    try
                    {
                        ...
                        MethodInfo methodInfo = typeof(TacticalActorBase).GetMethod("UpdateClassViewElementDefs", BindingFlags.Instance | BindingFlags.NonPublic);
